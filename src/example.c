...
int tench_test() {
    ...
    for (i = 0; i < TEST_NUM; i++) {
        ht_insert(&ht, &key[i], sizeof(clientUdpKey_t), NULL, 0);
    }
    ...
    for (i = 0; i < TEST_NUM; i++) {
        ht_get(&ht, &key[i], sizeof(clientUdpKey_t), NULL);
    }

    t2 = snap_time();
    fprintf(stderr, "Search %d keys took %.2f seconds\n", TEST_NUM, get_elapsed(t1, t2));
    return 0;
}
